...
namespace Plugin.WPF.Dock.AvalonDockMVVM
{
	/// <summary>
	...
	public partial class AvalonDockHost : UserControl
	{
		...
		private void AddPanel(object panel)
		{
			...
			if (managedContent == null)
			{
				...
				//
				this.contentMap.Add(panel, managedContent);
			}
			...
			try{
//				if((rand.Next() % 2) == 0){
//					(managedContent as DockableContent).ShowAsDocument(dockingManager);
//				} else {
				if(panel is IPluginUserControl)
				{
					managedContent.Name = ( panel as IPluginUserControl).Name.Replace(" ", "_");
				}
				(managedContent as DockableContent).Show(dockingManager, AnchorStyle.Top|AnchorStyle.Left|AnchorStyle.Bottom);
//				}
				managedContent.Hide();
			}catch(Exception ex)
			{
		...
		{
			try{
				if(this.contentMap.ContainsKey(panel))
				{
					var managedContent = this.contentMap[panel];
					(managedContent as DockableContent).Show(dockingManager, AnchorStyle.Left);
